...
int win_textbuffer_is_paging(window_t *win)
{
  ...
  /* If a zero-line window is paging, we'll never get to the end of it. */
  if (dwin->linesonpage <= 1) {
    gli_strict_warning("win_textbuffer_is_paging: would have scrolled zero lines");
    return FALSE;
  }
